...
parser.add_argument('-hw', '--half-width', default=0.001*0.15)
...
# halfwidth for nyc ortho imagery is about 0.001*0.15
...
radius = np.sqrt(2)*halfwidth*(1/np.cos(np.deg2rad(bbox[1][1]))) # the area to search for nearest neighbors
...
for kind, key in tqdm(enumerate(bins.keys()), total=len(bins)):
    ...
    for i, ind in enumerate(bins[key]):
        ...
        for nnind in nninds[0]:
            ...
            if (np.abs(nnpoint[0]-PoI[0])*(1/np.cos(np.deg2rad(PoI[1]))))<halfwidth and np.abs(nnpoint[1]-PoI[1])<halfwidth:
